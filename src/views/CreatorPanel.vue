<template>
  <div>
    <h1 class="text-3xl font-bold mb-6">Creator Panel</h1>
    
    <div v-if="!isConnected" class="p-6 bg-card rounded-lg shadow-sm border border-border mb-6">
      <p class="mb-4 text-muted-foreground">Please connect your wallet to access creator features.</p>
      <button 
        @click="connectWallet" 
    ...
    <div v-else class="grid gap-6">
      ...
      <div class="p-6 bg-card rounded-lg shadow-sm border border-border">
        <h2 class="text-xl font-semibold mb-4">Creator Status</h2>
        
        <div v-if="loading" class="text-muted-foreground">
          Checking creator status...
        </div>
        
        <div v-else-if="isCreator" class="text-green-600 font-medium">
          You are registered as a creator with {{ createdTokens.length }} token(s).
        </div>
        
        <div v-else class="text-muted-foreground">
          You are not yet registered as a creator. Mint a token to become a creator.
          <div class="mt-4">
            <router-link 
              to="/mint" 
              class="bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90 inline-block"
      ...
      <div v-if="isCreator" class="p-6 bg-card rounded-lg shadow-sm border border-border">
        <h2 class="text-xl font-semibold mb-4">Your Created Tokens</h2>
        
        <div v-if="loading" class="text-muted-foreground">
          Loading your tokens...
        </div>
        
        <div v-else-if="createdTokens.length === 0" class="text-muted-foreground">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full border-collapse">
            ...
            <tbody>
              <tr v-for="token in createdTokens" :key="token.id" class="border-b border-border">
      ...
      <!-- Royalty Information -->
      <div v-if="isCreator" class="p-6 bg-card rounded-lg shadow-sm border border-border">
        <h2 class="text-xl font-semibold mb-4">Royalty Information</h2>
        ...
        <div v-else>
          <p class="mb-4">
            Your tokens have the following royalty settings:
          </p>
          
          <ul class="list-disc pl-5 space-y-2">
            <li v-for="token in createdTokens" :key="token.id">
              Token #{{ token.id }}: {{ (Number(token.royaltyBps) / 100).toFixed(2) }}% royalty
            </li>
          </ul>
          
          <p class="mt-4 text-sm text-muted-foreground">
            Royalties are paid to the creator when tokens are sold on marketplaces that support ERC-2981.
          </p>
        </div>
      </div>
...
// Tokens created by the current user
const createdTokens = computed(() => {
